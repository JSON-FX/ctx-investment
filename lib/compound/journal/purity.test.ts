...
describe("journal purity", () => {
  it("has source files to check", () => {
    ...
    // (+ equity-series.ts) at Task 6. Raise this bound whenever a task adds a
    // top-level source file to this directory, so a scan that silently stops
    // finding a file fails loudly instead of passing on a smaller-than-
    // expected set.
    expect(sourceFiles().length).toBeGreaterThan(7);
  });
