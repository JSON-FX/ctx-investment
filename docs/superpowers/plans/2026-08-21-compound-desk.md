...
| **D-F** *(superseded — see below)* | ~~`requireManager()` enforces spec §9's AND gate in application code. Plan 3's P4 connects as `service_role`, which carries `BYPASSRLS`. RLS is therefore defence-in-depth for other clients and **not** what protects these pages.~~ | ~~Shipping pages that rely on a policy the connection bypasses would be the twelfth unfalsifiable safety net in this project. The gate is: signed in, `app_metadata.role = 'admin'`, **and** `account.managerUserId === user.id`.~~ |
| **D-F (updated, branch `fix/connect-as-authenticated`)** | **Every `compound_*` read and write now runs as `authenticated`, with the caller's own JWT claims (`withAuthenticatedDb`, `lib/compound/db/client.ts`), so the 16 policies in `compound_rls.sql` are the real boundary — not defence-in-depth for "other clients" that don't exist, the actual thing standing between one manager's rows and another's.** `requireAccount`/`requireManager` stay, as the second layer: verified by deleting `resolveOwnedAccount`'s own `managerUserId` comparison and re-running `gate.db.test.ts` + `cross-manager-isolation.db.test.ts` — all 30 assertions still passed with the application-level check removed, including "refuses another manager's account, even though it exists" and "role alone is not enough: an admin managing nothing here still cannot reach another manager's account." CopyTraderX's own tables (`deals`, `account_snapshots_daily`/`current`, `orders`, `positions`, `users`, `licenses`) are a separate, narrower case: RLS-enabled with **zero** policies and no grant to `authenticated` at all, so they stay on the elevated, read-only, named `withElevatedCopyTraderXRead` — 8 call sites total, enumerated with justification in `.superpowers/rls-connection-report.md`. | Production held 115 real ledger entries and the app was live on Vercel when this was found: the original D-F's own reasoning ("RLS is defence-in-depth... not what protects these pages") was true of the *code*, but had stopped being an acceptable trade the moment `requireAccount` became the *only* thing between managers' money, in production, with real users. The AND gate this row describes — signed in, `app_metadata.role = 'admin'`, **and** `account.managerUserId === user.id` — is unchanged; what changed is that it is no longer alone. |
